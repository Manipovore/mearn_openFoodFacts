...
class Packaging extends Component {
    constructor(props) {
        super(props);
        this.state = { apiResponse: [], dataCount: 10000, offset: 0 };
    }

    callApiData() {
        axios.get("http://localhost:9000/packaging/" + this.props.match.params.name + "/" + this.props.match.params.id)
            .then(res => this.setState({ apiResponse: res.data }))
            .catch(err => err);
    }

    callApiCount() {
        axios.get("http://localhost:9000/count/packaging_tags/" + this.props.match.params.name)
