...
class Origins extends Component {
    ...
    callApiData() {
        axios.get("http://localhost:9000/origins/" + this.props.match.params.name + "/" + this.props.match.params.id)
            .then(res => this.setState({ apiResponse: res.data }))
            .catch(err => err);
    }

    callApiCount() {
        axios.get("http://localhost:9000/count/origins/" + this.props.match.params.name)
